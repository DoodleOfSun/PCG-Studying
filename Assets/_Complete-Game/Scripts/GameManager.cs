...
namespace Completed
{
	...
	public class GameManager : MonoBehaviour
	{
		...
		public int playerHealth;							//Starting value for Player playerHealth points.
		public static GameManager instance = null;				//Static instance of GameManager which allows it to be accessed by any other script.
		[HideInInspector] public bool playersTurn = true;       //Boolean to check if it's players turn, hidden in inspector but public.
        public int enemySpawnRatio = 10;							// 적 생성 확률
		public bool enemiesFaster = false;						// 플레이어 공격력에 따른 적의 속도 증가 여부
		public bool enemiesSmarter = false;						// 플레이어 공격력에 따른 적의 향상된 AI 사용 여부
		...
		//GameOver is called when the player reaches 0 playerHealth points
		...
		IEnumerator MoveEnemies()
		{
			...
			for (int i = 0; i < enemies.Count; i++)
			{
				...
				{
                    if ((!enemies[i].getSpriteRenderer().isVisible) ||
                        (!boardScript.checkValidTile(enemies[i].transform.position)))
                    {
